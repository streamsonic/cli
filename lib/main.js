...
var path = require("path"),
	fs = require("fs"),
	util = require('util'),
	streamsonic = require("streamsonic");
...
Exec.prototype = {
	...
	user : function( domain, options ){
		var streamsonic = this.api.auth({key: this.creds.key ,secret: this.creds.secret, app: this.creds.app });
	...
	track : function( artist, options ){
		// fallbacks
		options = options || {};
		var streamsonic = this.api.auth({keyid: this.creds.key ,secret: this.creds.secret, app: this.creds.app });

		if( artist == "*" ){
			// read the domain list instead
			return streamsonic.list(function( error, result ) {
		...
		streamsonic.select( query, function( error, result ) {
			if( error ) return console.log( error );
			output( result );
		});
